...
- **ESP32-S3 Mini / PCA9685 driver board (THIS FORK'S RECOMMENDATION — zero soldering):** Uses a [Waveshare ESP32-S3-Zero-M](https://www.amazon.ca/dp/B0G43ZYD8G) (pre-soldered headers guaranteed) plus a ~$14 CAD [PCA9685 16-channel PWM driver board](https://www.amazon.ca/PCA9685-Interface-Controller-Compatible-Raspberry/dp/B07RMTN4NZ). Servos plug directly into the driver board's pre-soldered headers; only 4 wires (I2C) connect it to the S3 Mini. See [`docs/pca9685-fork/README.md`](../pca9685-fork/README.md) for the full guide and firmware notes.
- **Lolin S2 Mini / hand wiring (upstream default):** Uses a Lolin S2 Mini, loose headers, and point-to-point wiring straight to the ESP32's GPIO pins. Lowest cost, but the 3x8 servo header block is a genuinely difficult freehand solder job — demands patience to keep the harness tidy and leaves less room for mistakes. **Critical: Use 30AWG wire for data lines and 22AWG for power—larger gauge wire will make assembly nearly impossible.**
...
## How to wire the ESP32-S3 Mini / PCA9685 Driver Board (Recommended — No Soldering)

This is the wiring approach added by this fork. Instead of hand-building a 3×8 header/bus-wire array, you connect a single pre-assembled PCA9685 board and plug servos straight into it.

1. Wire the OLED to the S3 Mini: **GPIO8 = SDA, GPIO9 = SCL** (this fork's firmware default for the Waveshare ESP32-S3-Zero-M — note: *not* the upstream S2 Mini pins GPIO33/35; if you use an S2 Mini instead, uncomment its pin block in the firmware).
2. Connect 4 wires from the S3 Mini to the PCA9685 board:
   - S3 Mini 3V3 → PCA9685 VCC
   - S3 Mini GND → PCA9685 GND
   - S3 Mini GPIO8 → PCA9685 SDA
   - S3 Mini GPIO9 → PCA9685 SCL
   
   SDA/SCL can be shared with the OLED's existing I2C wires — it's a bus, both devices listen on the same two lines. Since the S3-Zero-M ships with pin headers pre-soldered, these are simple push-on jumper wires, no soldering required.
3. Connect your battery/5V power source to the PCA9685's separate screw terminal (servo power should not route through the S3 Mini).
4. Plug each servo's factory 3-pin connector into PCA9685 channels 0–7 in order ("motor 1" → channel 0, "motor 2" → channel 1, etc.) to match the existing leg/animation mapping.
5. Flash the firmware variant included in this fork (`firmware/sesame-firmware-main.ino` already targets the PCA9685 — see [`docs/pca9685-fork/README.md`](../pca9685-fork/README.md) for the library you need to install).

That's the entire servo wiring process — no protoboard, no bus wire, no soldering to the servo harness.

## How to wire the Lolin S2 Mini / Hand Wiring (Upstream Default)
