...
> [!NOTE]
> **PCA9685 Fork Users:** This fork's `sesame-firmware-main.ino` drives servos through a PCA9685 board over I2C instead of directly via `ESP32Servo`. Install **`Adafruit PWM Servo Driver Library`** instead of `ESP32Servo`. See [`docs/pca9685-fork/README.md`](../docs/pca9685-fork/README.md) for the wiring this pairs with.

- `Adafruit PWM Servo Driver Library` (PCA9685 fork) — or `ESP32Servo` **v3.0.9** if you're using the upstream hand-wired/direct-GPIO approach instead
- `Adafruit SSD1306`
- `Adafruit GFX Library`

> [!IMPORTANT]
> If using upstream's direct-GPIO approach, use **ESP32Servo v3.0.9** specifically. Newer releases currently have a known issue where writing to one servo can affect multiple channels ([madhephaestus/ESP32Servo#103](https://github.com/madhephaestus/ESP32Servo/issues/103)). This issue does not apply to the PCA9685 fork since it doesn't use the ESP32Servo library at all.
...
3. **Select your board**:
   ...
   - Go to **Tools → Board**
   - **For Waveshare ESP32-S3-Zero-M (this fork's recommendation):** Select "ESP32S3 Dev Module"
...
4. **Configure board settings**:
   ...
   - **For Waveshare ESP32-S3-Zero-M (this fork)**:
     - **USB CDC On Boot**: "Enabled" (Required for Serial Monitor / flashing over native USB-C)
...
6. **Choose your board configuration** in the code:
   ...
   - **If you built with the Waveshare ESP32-S3-Zero-M (this fork's default):** Nothing to change — the PCA9685 driver block and I2C pins (GPIO8/GPIO9) are already configured.
   - **If you built with the Lolin S2 Mini:** Uncomment the S2 Mini `I2C_SDA`/`I2C_SCL` defines (GPIO33/GPIO35). The PCA9685 driver block works with it as-is.
   - **If you built with the Distro Board V3/V2/V1:** This fork's main firmware targets the PCA9685; if you're building on a Distro Board instead, use upstream's firmware (or port the pin defines from upstream's `servoPins` arrays back in).
