...
class WasmChipRuntime:
    ...
    def __init__(
        ...
        # Reentrant entry lock guarding every wasm invocation (see _call_indirect).
    ...
    def _call_indirect(self, idx: int, *args: int) -> int:
        ...
        try:
            ...
            if fn is None:
                return 0
            # Serialize every entry into the chip's WASM. The QEMU thread
            # (I2C/UART callbacks) and the chip-timer thread can otherwise
            # enter the store concurrently; both then advance the same wasm
            # C stack pointer, smashing each other's frames until the stack
            # is exhausted ("call stack exhausted" traps under load).
            # RLock (not Lock) because chip callbacks legitimately re-enter
            # on the SAME thread (e.g. vx_pin_write -> pin watch dispatch).
    ...
    def fire_due_timers(self) -> None:
        ...
        for _i, t in due:
            prev_fire = t["next_fire_ns"]
            self._call_indirect(t["cb_idx"], t["user_data"])
            with self._timer_lock:
                if t["repeat"]:
                    # Resync to the present instead of advancing a single
                    # period per call. If the timer ever falls behind (e.g.
                    # the IO lock was held through a boot stall), += one
                    # period leaves it permanently overdue, free-running at
                    # scheduler loop speed and distorting chip PWM timing.
                    t["next_fire_ns"] = now + t["period_ns"]
                elif t["next_fire_ns"] == prev_fire:
                    # One-shot: only deactivate if the callback did NOT
                    # re-arm. Event-driven chips re-arm the next edge from
                    # inside their timer callback; the old unconditional
                    # active=False clobbered that re-arm and stalled the
                    # schedule after the first edge.
                    t["active"] = False
        self._flush_stdout()
