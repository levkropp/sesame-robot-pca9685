...
class _WorkerInstance:
    ...
    # Startup-crash retry state (see _thread_read_stdout's exit handler)
...
class EspLibManager:
    ...
    async def start_instance(
        ...
        inst.retry_count = (inst.start_kwargs or {}).get('retry_count', 0)
        with self._instances_lock:
    ...
    def _thread_read_stdout(self, inst: _WorkerInstance, client_id: str) -> None:
        ...
        try:
            ...
            if inst.running and rc is not None:
                fatal_signal = isinstance(rc, int) and rc < 0
                if fatal_signal and not inst.boot_seen and inst.retry_count < 3 and inst.start_kwargs:
                    # The esp32s3 machine init in this QEMU build segfaults
                    # intermittently (before any firmware code runs). Retry the
                    # launch instead of surfacing a crash banner — a retry
                    # usually gets a clean init.
                    inst.retry_count += 1
                    logger.warning('[%s] worker crashed during startup (code %s) — retrying (attempt %d/3)',
                                   client_id, rc, inst.retry_count)
                    kwargs = dict(inst.start_kwargs)
                    kwargs['retry_count'] = inst.retry_count
                    kwargs['callback'] = inst.callback
                    asyncio.run_coroutine_threadsafe(
                        self.start_instance(**kwargs), inst.loop)
                else:
