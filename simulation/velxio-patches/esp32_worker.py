    ...
    def _chip_timer_thread() -> None:
        while not _stopped.is_set():
            ...
            if soonest_ns is None:
                ...
                _stopped.wait(0.050)
                continue
            # Deadlines are expressed in each runtime's own sim clock
            # (sim_now_nanos, epoch = runtime construction). Computing "now"
            # from the worker's _t0_ref mixes two epochs and makes the wait
            # overshoot, after which fire_due_timers runs catch-up bursts
            # that distort chip-generated PWM edges. Use the runtime's own
            # clock for the wait computation instead.
            wait_ns = 0
            for rt in list(_chip_timer_runtimes):
                d = rt.next_timer_deadline()
                if d is not None:
                    w = d - rt.sim_now_nanos()
                    if w > wait_ns:
                        wait_ns = w
            wait_ns = max(0, wait_ns)
            if wait_ns > 0:
