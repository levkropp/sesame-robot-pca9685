...
// ============================================================================
// VELXIO SIMULATION BUILD
// This is a variant of sesame-firmware-main.ino for the Velxio emulator
// (see simulation/README.md in this repo). WiFi / captive portal / web server
// are stubbed out because the emulator can't meaningfully emulate a soft-AP.
// Everything else -- I2C OLED faces, PCA9685 servo driver, all animations,
// subtrim, and the Serial CLI -- is identical to the real firmware.
// Comment the line out to build the networking version again.
// ============================================================================
#define VELXIO_SIM
...
// I2C Pins for Lolin S2 Mini Board
//#define I2C_SDA 33
//#define I2C_SCL 35

// I2C Pins for ESP32-S3 Mini boards (e.g. Waveshare ESP32-S3-Zero-M) -- PCA9685 fork
// GPIO8/9 is Espressif's own conventional I2C default on ESP32-S3-DevKitC-1,
// safely away from strapping pins and not used by any onboard peripheral on
// the Waveshare S3-Zero (GPIO21 on that board drives the onboard WS2812 LED --
// avoid it for I2C even though some older docs suggest it).
#define I2C_SDA 8
#define I2C_SCL 9
...
// Servo Channels - PCA9685 16-Channel I2C PWM Driver Board
// ======================================================================
// MODIFIED: Servos are no longer wired directly to ESP32 GPIO pins.
// Instead, they plug directly into the PCA9685 board (no soldering to the
// S3 Mini needed for servos - just 4 wires between the S3 Mini and the
// PCA9685: VCC, GND, SDA, SCL - SDA/SCL share the same I2C bus as the OLED).
//
// Plug servo "motor 1" into PCA9685 channel 0, "motor 2" into channel 1,
// etc, matching the order below, to keep the exact same leg/animation
// mapping the rest of the firmware already expects.
// ======================================================================
Adafruit_PWMServoDriver pwm = Adafruit_PWMServoDriver(); // default I2C addr 0x40
#define PWM_FREQ 50 // 50Hz, standard analog servo frequency

// Servo channel order on the PCA9685 board (0-7)
const int servoPins[8] = {0, 1, 2, 3, 4, 5, 6, 7};

// Pulse width range (in microseconds) - same range the original firmware used
#define SERVO_MIN_US 732
#define SERVO_MAX_US 2929

// Converts a microsecond pulse width to the PCA9685's 12-bit tick count (0-4095) at 50Hz
uint16_t usToTicks(uint32_t us) {
  // Period at 50Hz = 20000us. 4096 ticks per period.
...
void setup() {
...
#else
  // VELXIO_SIM: no WiFi / captive portal / web server in the emulator.
  // The Serial CLI below is the way to drive animations instead.
  wifiInfoText = "VELXIO SIM  |  networking disabled  |  drive me from Serial Monitor: rn wf, rn wv, rn dn, rn rs, st  |  ";
  lastInputTime = millis();
  firstInputReceived = false;
  showingWifiInfo = false;
  Serial.println(F("VELXIO_SIM: WiFi/web/DNS/mDNS stubbed out; use the Serial CLI."));
#endif

  // PWM Init - PCA9685 board (shares the I2C bus already started above for the OLED)
  pwm.begin();
  ...
  Serial.println(F("Sesame boot complete."));
}
...
void setServoAngle(uint8_t channel, int angle) { 
  if (channel < 8) {
    ...
    // Map 0-180 degrees to the microsecond pulse range, then to PCA9685 ticks
    uint32_t pulseUs = SERVO_MIN_US + ((uint32_t)(SERVO_MAX_US - SERVO_MIN_US) * adjustedAngle) / 180;
    pwm.setPWM(servoPins[channel], 0, usToTicks(pulseUs));
